...
def solve_system(x, y, threshold=60, grid = np.zeros((N_x, M_x))):
    ...
    free_nodes  = coordinates[coordinates[:,3] == 1, 0].astype(int) # Bytta free_nodes  = ordering[ordering[:,2] == 1, 0].astype(int) til free_nodes  = coordinates[coordinates[:,3] == 1, 0].astype(int)

    fixed_nodes = coordinates[coordinates[:,3] == 0, 0].astype(int) # Bytta fixed_nodes = ordering[ordering[:,2] == 0, 0].astype(int) til fixed_nodes  = coordinates[coordinates[:,3] == 0, 0].astype(int)
