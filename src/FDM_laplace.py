...
# d^2u/self.dx^2 = (self.sound_pressure[i+1,j] - 2u[i,j] + self.sound_pressure[i-1,j]) / delta_x^2
# d^2u/self.dy^2 = (self.sound_pressure[i,j+1] - 2u[i,j] + self.sound_pressure[i,j-1]) / delta_y^2

# Letting delta_x = delta_y:
# => delta_u[i,j] = (self.sound_pressure[i+1,j] + self.sound_pressure[i-1,j] + self.sound_pressure[i,j+1] + self.sound_pressure[i,j-1]) / 4 - self.sound_pressure[i,j]

# Update self.sound_pressure
# self.sound_pressure[i,j] = self.sound_pressure[i,j] + delta_u[i,j]
...
class Room:
    def __init__(self):
        ...
        self.sound_pressure = np.zeros((self.grid_x+1, self.grid_y+1))
    ...
    def calculate_sound_pressure(self, tol=5e-4, max_iter=20000):
        # Boundary conditions
        self.sound_pressure[:, 0] = 0.0  # self.sound_pressure(x,0) = 0
        self.sound_pressure[:, self.grid_y] = 0.0  # self.sound_pressure(x,Ly) = 0
        self.sound_pressure[0, :] = 0.0  # self.sound_pressure(0,y) = 0
        self.sound_pressure[self.grid_x, :] = 0.0  # self.sound_pressure(Lx,y) = 0
        ...
        frame_index = 0
        self.sound_pressure_frames = np.zeros((self.sound_pressure.shape[0], self.sound_pressure.shape[1], max_iter // 10))
        self.sound_pressure_frames[:,:,frame_index] = self.sound_pressure.copy()
        for iteration in range(max_iter):
            self.sound_pressure_old = self.sound_pressure.copy()
            update = 0.0

            neighbor_left = self.sound_pressure_old[0:self.grid_x-1, 1:self.grid_y]
            neighbor_right = self.sound_pressure_old[2:self.grid_x+1, 1:self.grid_y]
            neighbor_down = self.sound_pressure_old[1:self.grid_x, 0:self.grid_y-1]
            neighbor_up = self.sound_pressure_old[1:self.grid_x, 2:self.grid_y+1]
            ...
            self.sound_pressure_new = (1.0 / denom) * ((right_effective + left_effective) / self.dx**2 + (up_effective + down_effective) / self.dy**2)
            interior_obstacles = self.obstacle_mask[1:self.grid_x, 1:self.grid_y]
            self.sound_pressure[1:self.grid_x, 1:self.grid_y] = np.where(interior_obstacles, 0.0, self.sound_pressure_new)
            for source in self.sources:
                self.sound_pressure[source.x, source.y] = source.strength

            delta = np.abs(self.sound_pressure - self.sound_pressure_old)
            update = np.max(delta[1:self.grid_x, 1:self.grid_y])

            if update < tol:
                print(f"Converged after {iteration + 1} iterations with max update {update}")
                self.sound_pressure = self.sound_pressure_frames
                return self.sound_pressure_frames, frame_index + 1, iteration + 1, update
            if iteration % 10 == 0:
                frame_index += 1
                self.sound_pressure_frames[:,:,frame_index] = self.sound_pressure.copy()
                if iteration % 1000 == 0:
        ...
        self.sound_pressure_frames = self.sound_pressure_frames[:,:,:frame_index+1]
        self.sound_pressure = self.sound_pressure_frames
        return self.sound_pressure_frames, frame_index + 1, max_iter, update

    def draw_frame(self, screen, frame, SCREEN_WIDTH, SCREEN_HEIGHT, paused=False):
        if self.sound_pressure is None:
            return 0.0

        if self.sound_pressure.ndim == 2:
            self.sound_pressure_frame = self.sound_pressure
        elif self.sound_pressure.ndim == 3:
            frame_index = frame % self.sound_pressure.shape[2]
            self.sound_pressure_frame = self.sound_pressure[:, :, frame_index]
        else:
            self.sound_pressure_frame = self.sound_pressure

        if np.all(self.sound_pressure_frame == 0):
            return 0.0


        self.sound_pressure_frame = np.flipud(np.where(self.sound_pressure_frame < 0.1, 0, self.sound_pressure_frame))

        covered = self.sound_pressure_frame > 0
        total_cells = covered.size
        covered_cells = np.sum(covered)

        percentage = 100 * covered_cells / total_cells
        
        self.sound_pressure_max = float(np.max(self.sound_pressure_frame))
        color_map_values = (255 * self.sound_pressure_frame / self.sound_pressure_max).astype(np.uint8)
